        ...
        <v-form 
          ...
          <v-card class="elevation-18 my-4">
            ...
            <v-card-actions v-if="!invalid_token">
              <v-btn color="primary" :loading="form1.loading" :disabled="form1.loading" large block type="submit">{{ $t('update_password') }}</v-btn>
            </v-card-actions>
          </v-card>
        </v-form>
      </v-flex>
...
<script>
  export default {
    ...
    created() {
      ...
      axios
        .post('/campaign/auth/password/reset/validate-token', {
    ...
    methods: {
      ...
      update(formName) {
        ...
        axios
          .post('/campaign/auth/password/update', {
            locale: this.$i18n.locale,
